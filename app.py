...
intensiteit_heen = st.number_input(
    "Motorvoertuig intensiteit heen (per etmaal)", min_value=0, value=1000
)

intensiteit_terug = st.number_input(
    "Motorvoertuig intensiteit terug (per etmaal)", min_value=0, value=0
)

snelheid_heen = st.number_input(
    "Snelheid motorvoertuigen heen (km/h)", min_value=0, value=60
)

snelheid_terug = st.number_input(
    "Snelheid motorvoertuigen terug (km/h)", min_value=0, value=60
)

fiets_h = st.number_input(
    "Fiets intensiteit heen (per etmaal)", min_value=0, value=20
)

fiets_t = st.number_input(
    "Fiets intensiteit terug (per etmaal)", min_value=0, value=20
)

fietsSpeedKmh = st.number_input(
    "Fiets snelheid (km/h)", min_value=0, value=18
)

length_km = st.number_input(
    "Weg lengte (km)", min_value=0.0, value=1.0
)
...
if st.button("Compute encounters"):
    ...
    st.metric(
        "Total encounters",
        f"{results['totalEncountersHour']:.0f}"
    )

    st.write("Breakdown:")
    st.write(f"🚗🚗 Car–Car: {results['ccEncountersHour']:.0f}")
    st.write(f"🚲🚗 Bike–Car: {results['bcEncountersHour']:.0f}")
    st.write(f"🚲🚲 Bike–Bike: {results['bbEncountersHour']:.0f}")
